...
class GCNCombiner(nn.Module):
    def __init__(self,
        ...
        if self.branch == 'species':
            total_num_selects = 2720
            num_joints = 85
            self.param_pool_hynode = nn.Linear(total_num_selects, num_joints)
            self.param_pool_hyedge = nn.Linear(total_num_selects, num_joints // 2)
        if self.branch == 'family':
            total_num_selects = 32
            num_joints = 85
            self.param_pool_hynode = nn.Linear(total_num_selects, num_joints)
            self.param_pool_hyedge = nn.Linear(total_num_selects, num_joints // 2)
        elif self.branch == 'order':
            total_num_selects = 32#2720
            num_joints = 85
            self.param_pool_hynode = nn.Linear(total_num_selects, num_joints)
            self.param_pool_hyedge = nn.Linear(total_num_selects, num_joints // 2)
    ...
    def forward(self, x):
        ...
        hs_hynode = self.param_pool_hynode(hs)
        hs_hyedge = self.param_pool_hyedge(hs)
        ### Hypergraph convolution
        x_alpha = self.conv_q1(hs_hynode).transpose(1, 2)
        x_gamma = self.conv_k1(hs_hynode).mean(2)
        x_gamma = torch.diag_embed(x_gamma)
        x_sigma = self.conv_k2(hs_hyedge)

        H = torch.matmul(x_alpha, x_gamma)
        H = torch.matmul(H, x_gamma.transpose(2, 1))
        H = torch.abs(torch.matmul(H, x_sigma))  # Incidence Matrix

        D = torch.diag_embed(torch.pow(H.mean(2), -0.5))  # Hypernode Degree
        B = torch.diag_embed(torch.pow(H.mean(1), -1))  # Hyperedge Degree

        DH = torch.matmul(D, H)
        G = torch.matmul(DH, torch.matmul(B, DH.transpose(2, 1)))
        ...
        hs = self.conv1(hs_hynode)
        # if torch.isnan(hs).any():
        #     hs = torch.where(torch.isnan(hs), torch.full_like(hs, torch.tensor(0, dtype=torch.float16) ), torch.tensor(0, dtype=torch.float16))
        # if torch.isinf(hs).any():
        #     hs = torch.where(torch.isinf(hs), torch.full_like(hs, 1), hs)
        # hs = hs - torch.matmul(hs, torch.matmul(DH, torch.matmul(B, DH.transpose(2, 1))))
        hs = hs - torch.matmul(hs, G)
        # if torch.isnan(hs).any():
        #     hs = torch.where(torch.isnan(hs), torch.full_like(hs, 0), 0)
        # if torch.isinf(hs).any():
        #     hs = torch.where(torch.isinf(hs), torch.full_like(hs, 1), hs)
        hs = self.batch_norm1(hs)
        if torch.isnan(hs).any():
...
class PluginMoodel(nn.Module):
    ...
    def __init__(self, 
        ...
        * num_selects:
        ...
        if self.use_combiner:
            ...
            self.combiner = GCNCombiner(total_num_selects, num_classes, gcn_inputs, gcn_proj_size, self.fpn_size, branch='species')
            #  JJJ
            family_num_classes= 38
            order_num_classes = 13
            family_num_selects = 32
            order_num_selects = 32#2048 + 512 + 128 +32
            self.combiner_family = GCNCombiner(family_num_selects, family_num_classes, gcn_inputs, gcn_proj_size, self.fpn_size, branch='family')
            self.combiner_order = GCNCombiner(order_num_selects, order_num_classes, gcn_inputs, gcn_proj_size, self.fpn_size, branch='order')
    ...
    def forward(self, x: torch.Tensor):
        ...
        if self.use_combiner:
            ...
            selects_order = {k : selects[k] for k in list(selects)[3:4]}
            selects_family = {k: selects[k] for k in list(selects)[3:4]}
            comb_outs_order = self.combiner_order(selects_order)
            comb_outs_family = self.combiner_family(selects_family)
            logits['comb_outs'] = comb_outs
            logits['comb_outs_order'] = comb_outs_order
            logits['comb_outs_family'] = comb_outs_family
            return logits
